...
# ── Manual add detection ──────────────────────────────────────────────────────

@router.message()
async def on_new_members_service(message: Message, bot: Bot, db: Database) -> None:
    """Detect when a user manually adds someone (service message new_chat_members)."""
    ...
    for new_member in message.new_chat_members:
        # Self-join via link/QR: from == new_member — skip
        if new_member.is_bot or new_member.id == adder.id:
            continue

        record = await db.get_user(message.chat.id, adder.id)
        if record and record.access_granted:
            continue

        if record is None:
            record = await db.create_user(message.chat.id, adder.id)
            try:
                await restrict(bot, message.chat.id, adder.id)
            except Exception:
                pass

        new_count = await db.increment_added(message.chat.id, adder.id)

        if new_count >= settings.required_members:
            await db.grant_access(message.chat.id, adder.id)
            await unrestrict(bot, message.chat.id, adder.id)
            importlib.reload(texts)
            await bot.send_message(
                message.chat.id,
                texts.SUCCESS.format(username=mention(adder.id, adder.first_name)),
                **thread(settings.topic_id),
            )

# ── Block unknown / restricted users ─────────────────────────────────────────
